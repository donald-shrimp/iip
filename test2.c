...
void processing(void) {
  ...
  // ヒストグラム付き画像を出力するよ
    ...
    for (x = 0; x < height; x++) {
      for (y = 0; y < width; y++) {
        if(x<(height)/2&&y>(width)/2){//右上 左上と同じ
          imgout[0][x][y] = tmp[x][y];
        }else if(x>(height)/2&&y<(width)/2){//左下　上下反転
          imgout[0][x][y] = tmp[height-1-x][y];
        }else if(x>(height)/2&&y>(width)/2){//右下　上下左右反転
          imgout[0][x][y] = tmp[height-1-x][width-1-y];
        }
      }
    }
