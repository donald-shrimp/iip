...
void processing(void) {
  int x, y, z, num, i, j, plus, low, k, insert = 0;
  //配列にコピーするよ
  printf("量子化サイズを入力してください(2,4,8,16)：");
  scanf("%d", &num);

  // numの二倍の数で割ってるよ
  plus = (int)256 / num;
  // lowはの判断基準の上限になるよ　plusを足すと上限になるよ
  low = 0;

  //この基準を満たす値だったらこの値を挿入...を基準を変えながら繰り返す動作をしているよ
  //指定した量子化サイズの数と同じ回数分繰り返すよ
  for (i = 0; i < num; i++) {
    insert = 0;
    for (k = 0; k < plus; k++) {
      insert += low + k;
    }
    insert = (int)(((double)insert / (double)plus) + 0.5);

    // printf("insert:%d", insert);
    for (x = 0; x < height; x++) {
      for (y = 0; y < width; y++) {
        imgout[0][x][y] = imgin[0][x][y];
        if (imgout[0][x][y] < low + plus && imgout[0][x][y] >= low) {
          imgout[0][x][y] = insert;
        }
      }
    }
