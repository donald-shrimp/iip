...
void processing(void) {
  int i,j,x,y,z;
  double percent,xd,yd,s,t;
double num,r1,r0;
  printf("拡大縮小率を入力してください：");
  scanf("%lf",&percent);
  ...
  for(i=0;i<height;i++){
      for(j=0;j<width;j++){
          ...
          s=xd-x;
          t=yd-y;
          if(x<width-1&&0<=x&&y<height-1&&0<=y){
          for(z=0;z<3;z++){
          r1=(1-s)*imgin[z][x][y+1]+s*imgin[z][x+1][y+1];
          r0=(1-s)*imgin[z][x][y]+s*imgin[z][x+1][y];
          imgout[z][i][j]=(int)(((1-t)*r0+t*r1)+0.5);
          }
      }else{
          imgout[0][i][j]=0;
          imgout[1][i][j]=128;
